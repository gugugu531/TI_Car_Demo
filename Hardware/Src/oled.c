...
#include <Hardware/Inc/oled.h>
#include <Hardware/Inc/oledfont.h>
#include <main.h>
...
u8 posX=0,posY=0;
u8 DotVal = 0;
for(posY=0;posY<64;posY++)
{
  for(posX=0;posX<128;posX++)
  {
   //DotVal = posX%2;
   if(posX%2)
	 {
   ...
   OLED_DrawPoint(posX,posY,DotVal);
  }
}
OLED_Refresh();
...
for(posY=0;posY<64;posY++)
{
  ...
  {
   OLED_DrawPoint(posX,posY,DotVal);
   //OLED_WR_Byte(OLED_GRAM[posX][posY],OLED_DATA);
  }
}
OLED_Refresh();
...
{
	if(num<0){
		OLED_ShowString(x-5, y, "-", size1, mode);
		OLED_Refresh();
		OLED_ShowNum(x,y,-num,len,size1,mode);
	}else{
	u8 t,temp,m=0;
	if(size1==8)m=2;
	for(t=0;t<len;t++)
...
//	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD, ENABLE);	 //ʹ��A�˿�ʱ��
